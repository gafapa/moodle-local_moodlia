...
The screenshots show that validation run. Release 0.1.206 uses the same web
...
- The site runs Moodle 5.2. MoodlIA 0.1.206 supports Moodle 5.2 only.
...
```text
empaquetado/local_moodlia-0.1.206.zip
```
...
The `MATURITY_BETA` message is informational for release 0.1.206. Stop if the
...
8. On **Plugins check**, confirm the following row before changing the
   ...
   - Required Moodle range: `502 - 502` for release 0.1.206
...
- Release: `0.1.206`, or the release you installed
- Version: `2026092001`, or the version declared by that release
- Source: **Additional**
...
Match the target Moodle release against `version.php`. MoodlIA 0.1.206 declares
