...
The screenshots show that validation run. Release 0.1.214 uses the same web
...
```text
empaquetado/local_moodlia-0.1.214.zip
```
...
The `MATURITY_BETA` message is informational for release 0.1.214. Stop if the
...
8. On **Plugins check**, confirm the following row before changing the
   ...
   - Required Moodle range: `405 - 503` for release 0.1.214
...
- Release: `0.1.214`, or the release you installed
...
Match the target Moodle release against `version.php`. MoodlIA 0.1.214 declares
