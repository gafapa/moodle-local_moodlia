...
/**
 * Reads Moodle Workshop grade information through Moodle external APIs.
 */
class get_workshop_grades {
    /**
    ...
    public static function execute(int $courseid, int $moduleid, int $userid = 0): array {
        ...
        $result = \mod_workshop_external::get_grades((int) $cm->instance, $resolveduserid);

        return [
            'course_id' => (int) $course->id,
        ] + workshop_tools::grades_to_response($cm, $resolveduserid > 0 ? $resolveduserid : (int) $USER->id, $result);
    }
}
